...
function processJSONChanges(lastCode, changesInput) {
    if (!lastCode.trim()) {
        return { errorMessage: 'Error: Empty code input' };
    }
    if (!changesInput.trim()) {
    ...
    try {
        ...
        return { errorMessage: 'Error: Invalid JSON format' };
    }
    ...
    for (const change of changes.reverse()) {
        const [start, end] = getLineRangeFromJSON(change, lines.length);
...
function getLineRangeFromJSON(change, totalLines) {
    const start = parseInt(change.lines.split('-')[0]) - 1;
    const end = change.lines.includes('-') ? parseInt(change.lines.split('-')[1]) - 1 : start;
    return [
        Math.max(0, Math.min(start, totalLines - 1)),
        Math.max(0, Math.min(end, totalLines - 1))
    ];
